...
| 1.1.4 | Fixed the resumed tutorial getting permanently stuck on "Select Case 00" (or "Open Case Management" / "Start the investigation") when the player resumed after already starting the investigation in an earlier session — those steps now count as already satisfied the moment Case 00 is the active investigation, since they must have already happened. |
| 1.1.5 | Fixed the screen appearing fully locked with nothing clickable on steps that highlight content loaded asynchronously (e.g. Evidence Database's evidence list) — the highlight now retries until the element exists instead of giving up on the first miss. |
| 1.1.6 | Fixed the tutorial's step-completion checks going out of sync with the real objective state shown in Case Management / Active Investigation (e.g. tutorial had moved on to Evidence while "Read Police Mail" was still unchecked). Steps that map to a real Case 00 objective now key off `objective:completed` — the same event those panels are driven by — instead of independently-derived proxy events. Also added a missing tutorial step (open the mail attachment) that the real objective chain requires but the dialogue previously skipped. |
| 1.1.7 | Follow-up polish for v1.1.6: three dialogue lines (evidence notes, CCTV bookmarking, board connections/theory) referenced actions as upcoming hints even though the preceding instruction step now already required the player to have done them. Reworded so the hint comes before the instruction and the line after it reads as a reinforcement, not a repeated instruction. |
